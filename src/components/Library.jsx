                ...
                <div className="grid grid-cols-3 md:grid-cols-4 pt-10 md:w-2/3">
                    {filteredBooks.map((book) => (
                        <div key={book.book.ISBN} className="flex flex-col items-center justify-center pb-10">
                            ...
                            <button onClick={() => handleAddBook(book.book.ISBN)} className='mt-2 rounded-full w-10 p-2 transition ease-in-out delay-150 bg-blue-500 hover:scale-110 hover:bg-indigo-500 duration-150'>
                                +
                            </button>
                        </div>
                    ))}
